...
function allSensorsForUi(){let base=(cache.sensors?.sensors||[]).map((s,i)=>Object.assign({type:s.type||'Capteur',_group:'sensors',_index:i},s));let ds=(cache.sensors?.ds18b20||[]).map((s,i)=>Object.assign({type:'DS18B20',source:'local',gpio:cache.sensors?.oneWireBus?.gpio||4,_group:'ds18b20',_index:i},s));return base.concat(ds)}
